...
export default function MapSelector({ gameType, selectedMap, onSelectMap }) {
  ...
  if (!gameType) {
    return (
      <div className="bg-white/10 backdrop-blur-md rounded-xl p-4">
        <p className="text-red-200 text-sm">Select game type first</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      <label className="block text-white font-medium">Select Map</label>
      <div className="grid grid-cols-2 gap-2">
        {maps.map((map) => {
          ...
          return (
            ...
            >
              {map.name}
              {map.premium && <span className="text-yellow-400">★</span>}
            </button>
          )
        })}
      </div>
